...
extern uc08 Cmd_judge_lora(char * dest);
...
//----------------ini-------------------------------------	
    ...
    tx_onoff485(INIT);
    tx_onoff485(OFF);
	UART0_op_selector(INIT);
	sbi(gfSystem_state,PORTINIT_IS);

	if(gfUse_iot == IOT_MODEM)
		UART0_op_selector(OFF);
  else UART0_op_selector(ON);  
...
void UART0_op_selector(char onoff){
	...
	{
	  case ON:
		  gpio_put(UART0_OPTION_SEL, 1);
		  break;
	  case OFF:
		  gpio_put(UART0_OPTION_SEL, 0);
		  break;
	  default: 
		  gpio_init(UART0_OPTION_SEL);
		  gpio_set_dir(UART0_OPTION_SEL,GPIO_OUT);
		  break;
	};
